//---------------------------------------------------------------------------------------
//  FILE:    X2Effect_WallClimbing.uc
//  AUTHOR:  TRNEEDANAME
//  PURPOSE: Create the Rock climb effect
//---------------------------------------------------------------------------------------


class X2Effect_WallClimbing extends X2Effect_PersistentTraversalChange;

var name WallClimbingEffectName;

function RegisterForEvents(XComGameState_Effect EffectGameState)
{
	local XComGameState_Unit EffectTargetUnit;
	local X2EventManager EventMgr;
	local XComGameStateHistory History;
	local Object ListenerObj;
	
	History = `XCOMHISTORY;
	EventMgr = `XEVENTMGR;
	
	//This object should handle the callback
	ListenerObj = self;

	EffectTargetUnit = XComGameState_Unit(History.GetGameStateForObjectID(EffectGameState.ApplyEffectParameters.TargetStateObjectRef.ObjectID));

	// Register for the required events
	EventMgr.RegisterForEvent(ListenerObj, 'UnitMoveFinished', WallClimbingAbilityActivated, ELD_OnStateSubmitted,, EffectTargetUnit);
}

function EventListenerReturn WallClimbingAbilityActivated(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	local XComGameStateContext_Ability AbilityContext;
	local XComGameStateContext_EffectRemoved EffectRemovedContext;
	local XComGameState_Effect WallClimbingEffectStateObject;
	local XComGameState NewGameState;
	local XComGameState_Unit AbilityUnit;
	
	AbilityUnit = XComGameState_Unit(EventSource);
	WallClimbingEffectStateObject = AbilityUnit.GetUnitAffectedByEffectState(WallClimbingEffectName);

	AbilityContext = XComGameStateContext_Ability(GameState.GetContext());
	if (WallClimbingEffectStateObject != none && 
		AbilityContext != none && AbilityContext.InputContext.MovementPaths[0].MovementTiles.Length > 0)
	{
		EffectRemovedContext = class'XComGameStateContext_EffectRemoved'.static.CreateEffectRemovedContext(WallClimbingEffectStateObject);
			
		NewGameState = `XCOMHISTORY.CreateNewGameState(true, EffectRemovedContext);
		WallClimbingEffectStateObject.RemoveEffect(NewGameState, NewGameState);			
		`TACTICALRULES.SubmitGameState(NewGameState);
	}

	return ELR_NoInterrupt;
}

DefaultProperties
{
	DuplicateResponse = eDupe_Ignore;
	WallClimbingEffectName = WallClimbing;
}